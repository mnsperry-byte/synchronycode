...
bounds = [ ...
    ...
    2.5265, ...
    2.6564 ...
]; %These are predetermined bounds for periods 1-2-4 regimes

rvals = [];
for i = 1:length(bounds)-1
    regime_vals = linspace(bounds(i),bounds(i+1),100);
    rvals = [rvals regime_vals];
end   % 100 simulations for each regime
fprintf('Total R values = %d\n',length(rvals))

% main loop storage 
lTheory = cell(size(rvals)); %space for theoretical prediction
lEmp = cell(size(rvals)); %space for simulation result 
...
for ir = 1:length(rvals)
    ...
    % step 2: extract empircal length scales for each reference phase in
    % the cycle
    ...
    % for phase in the period calculate a seperate empircal correlation
    % function
    lEmp_phase=zeros(period,1);

    for phase=1:period
        n_dec=n(t0+phase-1:period:end,:);
        maxdist=100;
        ...
        lEmp_phase(phase)=sqrt(abs(l2));
    end

    %Save all the length scales to corresponding r value
    lEmp{ir}=lEmp_phase;
    ...
    lTheory_phase=zeros(period,1);

    %Make theoretical predictions for each possible phase cycle
    for phase=1:period
        Nphase=circshift(Nstar,-(phase-1));
       
        %calculate relevant Floquet multipliers
        F=prod(fN(Nphase));
        A=zeros(period,1);
        for i=1:period
            if i==period
        ...
        F1_sq=fi(alpha1,Nphase).^2;
        F2_sq=fi(alpha2,Nphase).^2;
        ...
        lTheory_phase(phase)=sqrt(abs(lu2));
    end

    %save all length scales to corresponding r value
    lTheory{ir}=lTheory_phase;
end
